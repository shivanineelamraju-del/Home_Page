...
(function(){
  ...
  // Trace a single smooth curve through a set of points (Catmull-Rom-ish
  // quadratic smoothing) so the wisp reads as one continuous ribbon
  // instead of a chain of disconnected, seamed segments.
  ...
  function drawWisp(w, t){
    ...
    for(let i = 0; i < w.len; i++){
      ...
      const sway = Math.sin(t * w.freq + w.phase + i * 0.7) * w.amp
                 + Math.sin(t * w.freq * 0.4 + w.phase * 1.7 + i * 0.35) * w.amp * 0.35;
      pts.push({
        x: originX + sway,
        y: originY - along
      });
    ...
    grad.addColorStop(0, `hsla(${w.hue}, 60%, 62%, 0)`);
    grad.addColorStop(0.5, `hsla(${w.hue}, 62%, 66%, ${w.opacity})`);
    grad.addColorStop(1, `hsla(${w.hue + 10}, 70%, 74%, ${w.opacity * 0.7})`);

    ctx.save();
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    ctx.strokeStyle = grad;
    ctx.shadowColor = `hsla(${w.hue}, 65%, 60%, ${w.opacity * 1.4})`;

    // Layer a few passes, wide-and-soft to narrow-and-brighter, along the
    // *same* path so the ribbon feathers out cleanly at the edges.
    const layers = [
      { widthMul: 1.6, blur: 26, alpha: 0.5 },
      { widthMul: 1.0, blur: 14, alpha: 0.8 },
      { widthMul: 0.55, blur: 6,  alpha: 1.0 }
    ];
    layers.forEach(layer => {
      ctx.globalAlpha = layer.alpha;
      ctx.shadowBlur = layer.blur;
      ctx.lineWidth = w.width * layer.widthMul;
      traceSmoothPath(pts);
      ctx.stroke();
    });
    ctx.restore();
